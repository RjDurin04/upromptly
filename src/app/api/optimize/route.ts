 ...
 * Replaces @ts-expect-error directives with explicit type safety.
 */
...
export async function POST(request: NextRequest): Promise<Response> {
    ...
    try {
      ...
      if (!success) {
        logger.warn(
          {
            action: "rate_limit",
            ip: ip === "global-anonymous" ? undefined : ip,
            limit,
            remaining,
          },
          "Rate limit exceeded",
      ...
      logger.warn(
        { action: "rate_limit", err: summarizeError(error) },
        "Rate limiting failed. Executing fail-open path.",
      );
      // Fail open: if Upstash is down, we don't want to completely break the app.
    }
  } else {
    logger.warn(
      { action: "init_ratelimit" },
  ...
  if (!validationResult.success) {
    ...
    logger.warn(
      { action: "validate_request", issues: validationResult.error.issues },
    ...
    return Response.json(
      {
        error: {
          code: "INVALID_INPUT",
          message: firstIssue?.message ?? "Invalid request format.",
        },
      },
      { status: 400 },
    );
  ...
  if (!recaptchaSecretKey) {
    logger.error(
      { action: "init_recaptcha" },
      "Server configuration error: reCAPTCHA secret key is missing.",
    );
    return Response.json(
      { error: { code: "INVALID_STATE", message: "Internal server error." } },
      { status: 500 },
  ...
  try {
    ...
    if (recaptchaData.score !== undefined && recaptchaData.score < 0.5) {
      logger.warn(
        { action: "verify_recaptcha", score: recaptchaData.score },
    ...
    logger.error(
      { action: "verify_recaptcha", err: summarizeError(error) },
      "reCAPTCHA verification threw an error.",
    );
    return Response.json(
      {
        error: {
  ...
  try {
    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          logger.error(
            { action: "stream_response", err: summarizeError(streamingError) },
    ...
    logger.error(
      { action: "fetch_openrouter", err: summarizeError(fetchError) },
